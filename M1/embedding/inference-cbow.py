...
def get_word_vector_and_similar(word: str, tokenizer: Tokenizer, model: Word2Vec, topn: int = 20):
    ...
    Zwraca wektor i listę najbardziej podobnych tokenów.
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Model załadowany. Słownik zawiera {len(model.wv)} tokenów.")
    except FileNotFoundError:
        print(f"BŁĄD: Nie znaleziono pliku '{MODEL_FILE}'. Uruchom najpierw train-cbow.py, aby wytrenować model.")
        raise
