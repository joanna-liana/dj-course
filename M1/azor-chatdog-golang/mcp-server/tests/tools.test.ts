...
describe("ToolHandlers", () => {
  it("lists available tools", async () => {
    ...
    assert.ok(result.tools.some((t) => t.name === "delete_session"));
  ...
  it("calls delete_session tool", async () => {
    const originalDir = CONFIG.sessionDir;
    const testDir = join(process.cwd(), "tests", "fixtures");
    (CONFIG as any).sessionDir = testDir;

    const testSessionId = "test-delete-session";
    const testSessionPath = join(testDir, `${testSessionId}-log.json`);
    ...
    await writeFile(testSessionPath, JSON.stringify(testSessionData));

    let fileExists = true;
    try {
      await access(testSessionPath);
    } catch {
      fileExists = false;
    }
    ...
    const result = await handlers.callTool({
      params: {
        name: "delete_session",
        arguments: { session_id: testSessionId },
      },
      method: "tools/call",
    } as any);
